...
class TypeAttentiveEmbedder(KgeEmbedder):
    ...
    def _embed(self, embeds, type_embeds, type_padding_mask):
        query = embeds.unsqueeze(0)
        key = torch.cat((query, type_embeds), dim=0)
        value = key
        ...
        attn_output, attn_output_weights = self.self_attn(query, key, value, key_padding_mask=mask)
    ...
    def embed(self, indexes):
        ...
        type_embeds = self.type_embedder.embed(self.entity_types[indexes].T)
